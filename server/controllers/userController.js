...
if(req.param.id !== user.id.toString()) return res.status(400).json({message:"You cannot update other user's profile"});
...
const getUserProfile = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({message: err.message});
    console.log("Error in grtUserProfile:" ,err.message);
  }
}
